...
import { endTraining, getAmountPage, getResultSelector, pagesResultRead, startTraining, totalPages } from '../../redux/training/trainingSelectors';
// import { getAmountPage } from '../../redux/resultTraining/resultTrainingSelectors';


const StatisticChart = () => {
  const statisticRes = useSelector(getResultSelector).map((res)=>Number(res.pagesResult)) 
    ...
    const dayOfTraining = Math.floor((new Date(endDate)-new Date(dateStart)) / 1000/60/60/24) // кол-во полных дней
    const perDayPage = (totalPageNeedRead - totalPageRead) / (dayOfTraining - arrPageOfDay.length) // среднее кол-во страниц которое нужно прочитать 
    let arrDays = []
